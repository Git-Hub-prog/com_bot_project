...
import { demoOwner, demoSpace, demoTestimonials } from './data/demoData';
...
function AuthPanel({ mode, token }) {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', token: token || '' });
...
function Sidebar({ view, setView, space }) {
...
function EmbedPanel({ space }) {
  ...
  const widgetUrl = `${import.meta.env.VITE_APP_URL || window.location.origin}/widget.js`;
  const snippet = `<div id="testimonialhub-widget"></div>\n\n<script\n  src="${widgetUrl}"\n  data-space="${space.slug}"\n  data-layout="${options.layout}"\n  data-theme="${options.theme}"\n  data-show-ratings="${options.showRatings}"\n  data-show-avatars="${options.showAvatars}"\n  data-featured-only="${options.featuredOnly}"\n  data-count="${options.count}"\n  defer\n></script>`;
...
function Dashboard({ space, testimonials, setTestimonials, view, setView }) {
  const [query, setQuery] = useState('');
  const [filter, setFilter] = useState('all');
  const [toast, setToast] = useState('');
  const approved = testimonials.filter((item) => item.status === 'approved');
  const visible = useMemo(() => testimonials.filter((item) => (filter === 'all' || item.status === filter) && `${item.name} ${item.review} ${item.company}`.toLowerCase().includes(query.toLowerCase())), [testimonials, filter, query]);
  ...
  const moderate = async (id, action, featured) => {
    ...
    try { await apiRequest(`/testimonials/${id}/moderate`, { method: 'PATCH', body: { action, featured } }); } catch { /* demo fallback */ }
  ...
  if (view === 'embed') return <EmbedPanel space={space} />;
  ...
  return <div className="dashboard-view"><div className="page-heading"><div><span className="eyebrow">{view === 'inbox' ? 'Review queue' : 'Tuesday, September 17, 2026'}</span><h1>{view === 'inbox' ? 'Your inbox' : `Good morning, ${demoOwner.name.split(' ')[0]}.`}</h1><p>{view === 'inbox' ? 'Turn great customer stories into your best marketing asset.' : 'Here is what is happening with your social proof.'}</p></div><button className="button primary" onClick={() => setView('collect')}>Get testimonials <span>↗</span></button></div>{view === 'overview' && <div className="stats-grid"><StatCard label="Average rating" value={average} note="↑ 0.4 this month" accent="yellow" /><StatCard label="Total testimonials" value={testimonials.length} note="↑ 18% this month" accent="coral" /><StatCard label="Featured stories" value={testimonials.filter((item) => item.featured).length} note="Across your wall" accent="blue" /><StatCard label="Response rate" value="94%" note="↑ 6% this month" accent="green" /></div>}<section className="inbox-section"><div className="section-heading"><div><h2>{view === 'inbox' ? 'All testimonials' : 'Needs your attention'}</h2><p>{view === 'inbox' ? `${testimonials.length} stories in your collection` : 'Review, tag, and publish the latest customer stories.'}</p></div>{view === 'overview' && <button className="text-button" onClick={() => setView('inbox')}>View all <span>→</span></button>}</div><div className="toolbar"><div className="search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search testimonials..." /></div><div className="filter-tabs">{['all', 'pending', 'approved', 'archived'].map((item) => <button key={item} onClick={() => setFilter(item)} className={filter === item ? 'selected' : ''}>{item === 'all' ? 'All' : item[0].toUpperCase() + item.slice(1)}{item === 'pending' && <b>2</b>}</button>)}</div><button className="icon-button" title="Filter">≡</button></div><div className="review-list">{visible.map((review) => <ReviewRow key={review.id} review={review} onModerate={moderate} />)}{visible.length === 0 && <div className="empty-state">No testimonials match this filter.</div>}</div></section><Toast message={toast} /></div>;
}
...
function App() {
  ...
  const publicMode = route.name === 'collect' || route.name === 'wall';
  const routeSlug = path[1] || demoSpace.slug;
  const authMode = ['login', 'signup', 'verify-email', 'forgot-password', 'reset-password'].includes(route.name);
  const [space, setSpace] = useState(demoSpace);
  ...
  const setView = (nextView) => {
    ...
    const routeMap = { overview: '/dashboard', spaces: '/dashboard/spaces', new: '/dashboard/spaces/new', inbox: `/dashboard/spaces/${space.id}/reviews`, wall: `/dashboard/spaces/${space.id}/wall`, embed: `/dashboard/spaces/${space.id}/embed`, settings: `/dashboard/spaces/${space.id}/settings`, profile: '/dashboard/profile', collect: `/collect/${space.slug}` };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      if (authMode || route.name === 'home' || route.name === 'not-found') return;
      if (!publicMode) {
        try { await apiRequest('/auth/me'); setAuthReady(true); } catch { window.location.assign(ROUTES.login); return; }
      }
      try {
      ...
      } catch { /* demo data is the offline state */ }
    };
    load();
  ...
  }, [authMode, publicMode, route.name, routeSlug]);
  ...
  if (publicMode) return path[0] === 'collect' ? <PublicCollect space={space} /> : <PublicWall space={space} testimonials={testimonials} />;
  ...
  if (!authReady) return <main className="public-shell"><div className="public-card success-card"><p className="eyebrow">Account access</p><h1>Checking your session...</h1><p className="muted">Please wait while we securely open your workspace.</p></div></main>;
